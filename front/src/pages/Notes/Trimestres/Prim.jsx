...
const Prim = ({type}) => {
    ...
    const handleEdit = (value, student_id, subject_id, subject_type) => {
        const data = {
            value,
            student_id,
            subject_id,
        ...
        fetch(host+'/notes/addOrUpdate2', {method: 'POST', body: JSON.stringify(data), 
            headers: {'Content-Type': 'application/json', 'Authorization': sessionStorage.user}})
            .then((res) => res.json())
            .then(res => {
                if (!res.success) {
        ...
        <nav className="navbar navbar-expand-lg" style={{padding: '10px'}}>
            <div className="collapse navbar-collapse" id="navbarNav">
                <ul className="navbar-nav" style={{fontSize: '1.3rem', display:"flex", justifyContent:'space-between'}}>
                    ...
                    <a href={host+`/download/pdf/bul2/${ActualClass.year}/`+class_id+'/'+exam_id} target={'_blank'} className="btn btn-success">{sequenceTraductions['fr'].downloadBuls}</a>
                    ...
                    return <tr key={index}>
                        ...
                        
                        {
                            subjects.map((subject, id) => {
                                ...
                                return <>
                                    <td key={id}>
                                        <input style={{width: '50px'}} type="number" defaultValue={note1} 
                                        onChange={(e) => {handleEdit(e.target.value, student.id, subject.id, 'devoir')}} />
                                    </td> 
                                    <td key={id}>
                                        <input style={{width: '50px'}} type="number" defaultValue={note2} 
                                        onChange={(e) => {handleEdit(e.target.value, student.id, subject.id, 'compo')}} />
                                    </td> 
                                    <td>
                                        {
                                            t2 / 2
                        ...
                        <td>
                            ...
                            <a target={'_blank'} href={`${host}/download/pdf/bul2/${ActualClass.year}/${class_id}/${student.id}/${exam_id}`} 
